...
if [ -z "$DB_ROOT_PASSWORD" ] || [ -z "$DB_PASSWORD" ] || [ -z "$DB_NAME" ] \
     || [ -z "$DB_USER" ] || [ -z "$WP_TITLE" ] || [ -z "$WP_ADMIN_USERNAME" ] \
     || [ -z "$WP_ADMIN_PASSWORD" ] || [ -z "$WP_ADMIN_EMAIL" ] || [ -z "$WP_USER_USERNAME" ] || [ -z "$WP_USER_EMAIL" ] || [ -z "$WP_USER_PASSWORD" ] ; then
    echo "DB_ROOT_PASSWORD, DB_PASSWORD, must be set"
    exit 1
fi
